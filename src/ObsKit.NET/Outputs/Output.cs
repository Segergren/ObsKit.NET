...
public class Output : ObsObject
{
    ...
    /// <summary>
    /// Registers a pair of mutually-exclusive hotkeys tied to this output (e.g. enable/disable).
    /// When both share a key combination, only the handler that returns true consumes the press.
    /// Dispose the returned object to unregister; it is also unregistered when the output is destroyed.
    /// </summary>
    ...
    public void LoadHotkeys(Settings hotkeys)
    {
        ...
        ObsHotkey.obs_hotkeys_load_output(Handle, hotkeys.Handle);
    }
